...
class TransferLearningHelper(
        ...
        processInputImage(bitmap, rotation)?.let { image ->
            synchronized(lock) {
                ...
                val output = TensorBuffer.createFixedSize(
                    intArrayOf(1, 5),  // Model now supports 5 classes
                    DataType.FLOAT32
                )
    ...
    private fun encoding(id: Int): FloatArray {
        val classEncoded = FloatArray(5) { 0f }  // Model now supports 5 classes
        classEncoded[id] = 1f
        return classEncoded
    }
    ...
    companion object {
        ...
        private val classes = mapOf(
            ...
            CLASS_THREE to 2,
            CLASS_FOUR to 3,
            CLASS_FIVE to 4  // Now supported - model has 5 classes
        )
